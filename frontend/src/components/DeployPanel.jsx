...
export default function DeployPanel({ signer }) {
    ...
    const loadArtifacts = async () => {
      try {
        const [multiSigArtifact, myContractArtifact, erc1967ProxyArtifact] = await Promise.all([
          ...
          fetch('/artifacts/MyContractV1.json').then(res => res.json()),
          fetch('/artifacts/ERC1967Proxy.json').then(res => res.json()).catch(() => {
  ...
  const deployContract = async () => {
    ...
    try {
      ...
      setStatus(`MultiSig deployed to: ${multiSigAddress}\nNow deploying UUPS implementation...`);

      // 2. Deploy Implementation Contract (MyContractV1)
      setStatus(prev => `${prev}\nDeploying MyContractV1 implementation...`);
      ...
      // 3. Deploy ERC1967Proxy for UUPS pattern
      setStatus(prev => `${prev}\nDeploying and initializing proxy...`);

      // Create initialization data for the proxy
      // Based on MyContractV1.sol, initialize takes an owner address
      const initializeData = implementation.interface.encodeFunctionData("initialize", [signerAddress]);

      // Use the ERC1967Proxy contract to create a proxy pointing to our implementation
      const ProxyFactory = new ethers.ContractFactory(
        contractArtifacts.erc1967Proxy.abi,
        contractArtifacts.erc1967Proxy.bytecode,
        signer
      );
      ...
      // Create a contract instance with the proxy address but using the implementation ABI
      ...
      try {
        const tx = await proxyAsImpl.transferOwnership(multiSigAddress);
        await tx.wait();
